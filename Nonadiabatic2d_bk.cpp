...
void Nonadiabatic2d::Evolve()
{
    ...
    if ( !isFullGrid )
    {
        ...
        #pragma omp parallel for reduction(+: ta_size_A) private(b1,b2,b3) schedule(runtime)
        for (int i1 = EDGE; i1 < BoxShape[0]-EDGE; i1 ++)  {
            for (int i2 = EDGE; i2 < BoxShape[1]-EDGE; i2 ++)  {
                b1 = PF_A[i1*W1+i2] < TolH_A;
                b2 = std::abs(F_A[(i1+1)*W1+i2] - F_A[(i1-1)*W1+i2]) < TolHdX1_A;
                b3 = std::abs(F_A[i1*W1+(i2+1)] - F_A[i1*W1+(i2-1)]) < TolHdX2_A;
        ...
        // Truncation and TA
        #pragma omp parallel for reduction(+: ta_size_B) private(b1,b2,b3) schedule(runtime)
        for (int i1 = EDGE; i1 < BoxShape[0]-EDGE; i1 ++)  {
            for (int i2 = EDGE; i2 < BoxShape[1]-EDGE; i2 ++)  {
                b1 = PF_B[i1*W1+i2] < TolH_B;
                b2 = std::abs(F_B[(i1+1)*W1+i2] - F_B[(i1-1)*W1+i2]) < TolHdX1_B;
                b3 = std::abs(F_B[i1*W1+(i2+1)] - F_B[i1*W1+(i2-1)]) < TolHdX2_B;
    ...
    for (int tt = 0; tt < (int)(TIME / kk); tt ++)
    {
        ...
        if ( !isFullGrid )
        {
            ...
            #pragma omp parallel for reduction(merge: tmpVec) private(g1,g2,b1,b2,b3,b4,b5) schedule(runtime)
            for (int i = 0; i < TB_A.size(); i++)
            {
                g1 = (int)(TB_A[i] / M1);
                g2 = (int)(TB_A[i] % M1);
                b1 = PF_A[g1*W1+g2] >= TolL_A;
                b2 = std::abs(F_A[(g1+1)*W1+g2] - F_A[(g1-1)*W1+g2]) >= TolLdX1_A;
                b3 = std::abs(F_A[g1*W1+(g2+1)] - F_A[g1*W1+(g2-1)]) >= TolLdX2_A;
                b4 = g1 > EDGE && g2 > EDGE;
            ...
            for (int i = 0; i < TB_B.size(); i++)
            {
                ...
                b2 = std::abs(F_B[(g1+1)*W1+g2] - F_B[(g1-1)*W1+g2]) >= TolLdX1_B;
                b3 = std::abs(F_B[g1*W1+(g2+1)] - F_B[g1*W1+(g2-1)]) >= TolLdX2_B;
                b4 = g1 > EDGE && g2 > EDGE;
                b5 = g1 < BoxShape[0]-EDGE-1 && g2 < BoxShape[1]-EDGE-1;
        ...
        while ( !isFullGrid && (TBL_A.size() != 0 || TBL_B.size() != 0) && (Excount_A < ExLimit && Excount_B < ExLimit) )
        {
            ...
            if ( isFirstExtrp_A && isFirstExtrp_B )  {
                ...
                #pragma omp parallel 
                {
                    ...
                    // RK4-1
                    #pragma omp for private(xx1,xx2,f0_A,f1p1_A,f1m1_A,f2p1_A,f2m1_A,f0_B,f1p1_B,f1m1_B,f2p1_B,f2m1_B,c_f0_a,c_f0_b,pot_a,pot_b,pot_c) schedule(runtime)
                    ...
                    // RK4-2
                    #pragma omp for private(xx1,xx2,f0_A,f1p1_A,f1m1_A,f2p1_A,f2m1_A,kk0_A,kk1p1_A,kk1m1_A,kk2p1_A,kk2m1_A,f0_B,f1p1_B,f1m1_B,f2p1_B,f2m1_B,kk0_B,kk1p1_B,kk1m1_B,kk2p1_B,kk2m1_B,c_f0_a,c_f0_b,pot_a,pot_b,pot_c) schedule(runtime)
                    for (int i1 = x1_min_C; i1 <= x1_max_C; i1 ++)  {
                        for (int i2 = x2_min_C; i2 <= x2_max_C; i2 ++)  {
                            if (TAMask_A[i1*W1+i2] || TAMask_B[i1*W1+i2])  {
                                xx1 = Box[0] + i1 * H[0];
            ...
            else
            {
                ...
                if ( ExFF_A.size() > 0 || ExFF_B.size() > 0 )  {
                    ...
                    // RK4-1
                    #pragma omp parallel for private(g1,g2,xx1,xx2,f0_A,f1p1_A,f1m1_A,f2p1_A,f2m1_A,f0_B,f1p1_B,f1m1_B,f2p1_B,f2m1_B,c_f0_a,c_f0_b,pot_a,pot_b,pot_c) schedule(runtime)
                    ...
                    // RK4-2
                    #pragma omp for private(g1,g2,xx1,xx2,f0_A,f1p1_A,f1m1_A,f2p1_A,f2m1_A,kk0_A,kk1p1_A,kk1m1_A,kk2p1_A,kk2m1_A,f0_B,f1p1_B,f1m1_B,f2p1_B,f2m1_B,kk0_B,kk1p1_B,kk1m1_B,kk2p1_B,kk2m1_B,c_f0_a,c_f0_b,pot_a,pot_b,pot_c) schedule(runtime)
                    for (int i = 0; i < ExBD.size(); i++)  {
            ...
            if ( ExFF_A.size() > 0 )  {
                ...
                for (int i = 0; i < ExFF_A.size(); i++)
                {
                    if (Check_A[i])  {
                        g1 = (int)(ExFF_A[i] / M1);
                        g2 = (int)(ExFF_A[i] % M1);
                        b1 = std::abs(FF_A[g1*W1+g2] * std::conj(FF_A[g1*W1+g2])) >= TolH_A;
                        b2 = std::abs(FF_A[(g1+1)*W1+g2] - FF_A[(g1-1)*W1+g2]) >= TolHdX1_A;
                        b3 = std::abs(FF_A[g1*W1+(g2+1)] - FF_A[g1*W1+(g2-1)]) >= TolHdX2_A;
                        b4 = g1 > EDGE && g2 > EDGE;
                        b5 = g1 < BoxShape[0]-EDGE-1 && g2 < BoxShape[1]-EDGE-1;
            ...
            if ( ExFF_B.size() > 0 )  {
                ...
                #pragma omp parallel for reduction(merge: tmpVec) private(g1,g2,b1,b2,b3) schedule(runtime)
                for (int i = 0; i < ExFF_B.size(); i++)  {
                    if (Check_B[i])  {

                        g1 = (int)(ExFF_B[i] / M1);
                        g2 = (int)(ExFF_B[i] % M1);
                        b1 = std::abs(FF_B[g1*W1+g2] * std::conj(FF_B[g1*W1+g2])) >= TolH_B;
                        b2 = std::abs(FF_B[(g1+1)*W1+g2] - FF_B[(g1-1)*W1+g2]) >= TolHdX1_B;
                        b3 = std::abs(FF_B[g1*W1+(g2+1)] - FF_B[g1*W1+(g2-1)]) >= TolHdX2_B;
                        b4 = g1 > EDGE && g2 > EDGE;
                        b5 = g1 < BoxShape[0]-EDGE-1 && g2 < BoxShape[1]-EDGE-1;
        ...
        if (!isFullGrid)  {
            ...
            #pragma omp parallel for reduction (+:norm_A) schedule(runtime)
            for (int i1 = EDGE; i1 < BoxShape[0]-EDGE-1; i1 ++)  {
                for (int i2 = EDGE; i2 < BoxShape[1]-EDGE-1; i2 ++)  {
                    norm_A += std::abs(FF_A[i1*W1+i2]*std::conj(FF_A[i1*W1+i2]));
                }
            }
        }
        ...
        if ( !isFullGrid )
        {
                ...
                for (int i2 = x2_min_C; i2 <= x2_max_C; i2 ++)  {
                    if (TAMask_A[i1*W1+i2])  {
                        if (PF_A[i1*W1+i2] < TolH_A)  {
                            ...
                            bool b1 = std::abs(FF_A[(i1+1)*W1+i2] - FF_A[(i1-1)*W1+i2]) < TolHdX1_A;
                            bool b2 = std::abs(FF_A[i1*W1+(i2+1)] - FF_A[i1*W1+(i2-1)]) < TolHdX2_A;
                ...
                for (int i2 = x2_min_C; i2 <= x2_max_C; i2 ++)  {
                    if (TAMask_B[i1*W1+i2])  {
                       if (PF_B[i1*W1+i2] < TolH_B)  {
                            ...
                            bool b1 = std::abs(FF_B[(i1+1)*W1+i2] - FF_B[(i1-1)*W1+i2]) < TolHdX1_B;
                            bool b2 = std::abs(FF_B[i1*W1+(i2+1)] - FF_B[i1*W1+(i2-1)]) < TolHdX2_B;
            ...
            #pragma omp parallel for reduction(min: x1_min_A,x2_min_A) reduction(max: x1_max_A,x2_max_A) reduction(+: ta_size_A) schedule(runtime)
            for (int i1 = x1_min_C; i1 <= x1_max_C; i1 ++)  {
                for (int i2 = x2_min_C; i2 <= x2_max_C; i2 ++)  {
                    if (TAMask_A[i1*W1+i2])  {
                        if (i1 < x1_min_A)  x1_min_A = i1;
            ...
            #pragma omp parallel for reduction(min: x1_min_B,x2_min_B) reduction(max: x1_max_B,x2_max_B) reduction(+: ta_size_B) schedule(runtime)
            for (int i1 = x1_min_C; i1 <= x1_max_C; i1 ++)  {
                for (int i2 = x2_min_C; i2 <= x2_max_C; i2 ++)  {
                    if (TAMask_B[i1*W1+i2])  {
                        if (i1 < x1_min_B)  x1_min_B = i1;
