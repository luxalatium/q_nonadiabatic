...
void Nonadiabatic2d::Evolve()
{
    ...
    if ( !isFullGrid )
    {
        ...
        #pragma omp parallel for reduction(+: ta_size_A) private(b1,b2,nx1,nx2,\
                                                                f1p1_A,f1m1_A,f2p1_A,f2m1_A) schedule(runtime)
        for (int i1 = EDGE; i1 < BoxShape[0]-EDGE; i1 ++)  {
            for (int i2 = EDGE; i2 < BoxShape[1]-EDGE; i2 ++)  {
                nx1 = int(TAMask_A[(i1+1)*W1+i2]) + int(TAMask_A[(i1-1)*W1+i2]);
                nx2 = int(TAMask_A[i1*W1+(i2+1)]) + int(TAMask_A[i1*W1+(i2-1)]);

                f1p1_A = (TAMask_A[(i1+1)*W1+i2]) ? F_A[(i1+1)*W1+i2] : F_A[i1*W1+i2];
                f1m1_A = (TAMask_A[(i1-1)*W1+i2]) ? F_A[(i1-1)*W1+i2] : F_A[i1*W1+i2];
                f2p1_A = (TAMask_A[i1*W1+(i2+1)]) ? F_A[i1*W1+(i2+1)] : F_A[i1*W1+i2];
                f2m1_A = (TAMask_A[i1*W1+(i2-1)]) ? F_A[i1*W1+(i2-1)] : F_A[i1*W1+i2];

                b1 = PF_A[i1*W1+i2] < TolH_A;
                b2 = ((nx1 == 0) ? 0.0 : pow(std::abs(f1p1_A - f1m1_A)/(nx1*H[0]),2)) + \
                     ((nx2 == 0) ? 0.0 : pow(std::abs(f2p1_A - f2m1_A)/(nx2*H[1]),2)) < TolHd_A_sq;
        ...
        if (ta_size_A == 0)  {
            ...
            for (int i1 = x1_min_A; i1 <= x1_max_A; i1 ++)  {
                for (int i2 = x2_min_A; i2 <= x2_max_A; i2 ++)  {
                    if (TAMask_A[i1*W1+i2] && ( \
                        !TAMask_A[(i1+1)*W1+i2] || !TAMask_A[(i1-1)*W1+i2] || \
                        !TAMask_A[i1*W1+(i2+1)] || !TAMask_A[i1*W1+(i2-1)]))
            ...
            for (int i = 0; i < TB_A.size(); i++)  {
                ...
                if ( g1+1 < BoxShape[0]-EDGE-1 && !TAMask_A[(g1+1)*W1+g2])
                    tmpVec.push_back(GridToIdx(g1+1,g2));
                if ( g1-1 > EDGE && !TAMask_A[(g1-1)*W1+g2])
                    tmpVec.push_back(GridToIdx(g1-1,g2));
            ...
            #pragma omp parallel for reduction(min: x1_min_A,x2_min_A) \
                                     reduction(max: x1_max_A,x2_max_A) \
        ...
        // Truncation and TA
        #pragma omp parallel for reduction(+: ta_size_B) private(b1,b2,nx1,nx2,\
                                                                f1p1_B,f1m1_B,f2p1_B,f2m1_B) schedule(runtime)
        for (int i1 = EDGE; i1 < BoxShape[0]-EDGE; i1 ++)  {
            for (int i2 = EDGE; i2 < BoxShape[1]-EDGE; i2 ++)  {

                nx1 = int(TAMask_B[(i1+1)*W1+i2]) + int(TAMask_B[(i1-1)*W1+i2]);
                nx2 = int(TAMask_B[i1*W1+(i2+1)]) + int(TAMask_B[i1*W1+(i2-1)]);
                
                f1p1_B = (TAMask_B[(i1+1)*W1+i2]) ? F_B[(i1+1)*W1+i2] : F_B[i1*W1+i2];
                f1m1_B = (TAMask_B[(i1-1)*W1+i2]) ? F_B[(i1-1)*W1+i2] : F_B[i1*W1+i2];
                f2p1_B = (TAMask_B[i1*W1+(i2+1)]) ? F_B[i1*W1+(i2+1)] : F_B[i1*W1+i2];
                f2m1_B = (TAMask_B[i1*W1+(i2-1)]) ? F_B[i1*W1+(i2-1)] : F_B[i1*W1+i2];

                b1 = PF_B[i1*W1+i2] < TolH_B;
                b2 = ((nx1 == 0) ? 0.0 : pow(std::abs(f1p1_B - f1m1_B)/(nx1*H[0]),2)) + \
                     ((nx2 == 0) ? 0.0 : pow(std::abs(f2p1_B - f2m1_B)/(nx2*H[1]),2)) < TolHd_B_sq;
        ...
        if (ta_size_B == 0)  {
            ...
            for (int i1 = x1_min_B; i1 <= x1_max_B; i1 ++)  {
                for (int i2 = x2_min_B; i2 <= x2_max_B; i2 ++)  {
                    if (TAMask_B[i1*W1+i2] && ( \
                        !TAMask_B[(i1+1)*W1+i2] || !TAMask_B[(i1-1)*W1+i2] || \
                        !TAMask_B[i1*W1+(i2+1)] || !TAMask_B[i1*W1+(i2-1)]))
            ...
            for (int i = 0; i < TB_B.size(); i++)  {
                ...
                if ( g1+1 < BoxShape[0]-EDGE-1 && !TAMask_B[(g1+1)*W1+g2])
                    tmpVec.push_back(GridToIdx(g1+1,g2));
                if ( g1-1 > EDGE && !TAMask_B[(g1-1)*W1+g2])
                    tmpVec.push_back(GridToIdx(g1-1,g2));
            ...
            // Update TA box
            #pragma omp parallel for reduction(min: x1_min_B,x2_min_B) \
                                     reduction(max: x1_max_B,x2_max_B) \
                                     private(g1,g2) schedule(runtime)
            for (int i = 0; i < tmpVec.size(); i ++)  {
                g1 = (int)(tmpVec[i] / M1);
                g2 = (int)(tmpVec[i] % M1);
    ...
    for (int tt = 0; tt < (int)(TIME / kk); tt ++)
    {
                ...
                for (int i1 = x1_min_A; i1 <= x1_max_A; i1 ++)  {
                    for (int i2 = x2_min_A; i2 <= x2_max_A; i2 ++)  {
                        if (TAMask_A[i1*W1+i2])  {
                            xx1 = Box[0] + i1 * H[0];
                            xx2 = Box[2] + i2 * H[1];
                            fprintf(pfile, "%d %d %lf %lf %.16e %.16e\n", i1, i2, xx1, xx2, real(F_A[i1*W1+i2]), imag(F_A[i1*W1+i2]));
                        }
                    }
                }
                fclose(pfile);
                ...
                for (int i1 = x1_min_B; i1 <= x1_max_B; i1 ++)  {
                    for (int i2 = x2_min_B; i2 <= x2_max_B; i2 ++)  {
                        if (TAMask_B[i1*W1+i2])  {
                            xx1 = Box[0] + i1 * H[0];
                            xx2 = Box[2] + i2 * H[1];
                            fprintf(pfile, "%d %d %lf %lf %.16e %.16e\n", i1, i2, xx1, xx2, real(F_B[i1*W1+i2]), imag(F_B[i1*W1+i2]));
                        }
                    }
                }
                fclose(pfile);
        ...
        if ( !isFullGrid )
        {
            ...
            for (int i = 0; i < TB_A.size(); i++)
            {
                ...
                nx1 = int(TAMask_A[(g1+1)*W1+g2]) + int(TAMask_A[(g1-1)*W1+g2]);
                nx2 = int(TAMask_A[g1*W1+(g2+1)]) + int(TAMask_A[g1*W1+(g2-1)]);
                
                f1p1_A = (TAMask_A[(g1+1)*W1+g2]) ? F_A[(g1+1)*W1+g2] : F_A[g1*W1+g2];
                f1m1_A = (TAMask_A[(g1-1)*W1+g2]) ? F_A[(g1-1)*W1+g2] : F_A[g1*W1+g2];
                f2p1_A = (TAMask_A[g1*W1+(g2+1)]) ? F_A[g1*W1+(g2+1)] : F_A[g1*W1+g2];
                f2m1_A = (TAMask_A[g1*W1+(g2-1)]) ? F_A[g1*W1+(g2-1)] : F_A[g1*W1+g2];

                b1 = PF_A[g1*W1+g2] >= TolL_A;
                b2 = ((nx1 == 0) ? 0.0 : pow(std::abs(f1p1_A - f1m1_A)/(nx1*H[0]),2)) + \
                     ((nx2 == 0) ? 0.0 : pow(std::abs(f2p1_A - f2m1_A)/(nx2*H[1]),2)) >= TolLd_A_sq;
                b3 = g1 > EDGE && g2 > EDGE;
            ...
            for (int i = 0; i < TB_B.size(); i++)
            {
                ...
                nx1 = int(TAMask_B[(g1+1)*W1+g2]) + int(TAMask_B[(g1-1)*W1+g2]);
                nx2 = int(TAMask_B[g1*W1+(g2+1)]) + int(TAMask_B[g1*W1+(g2-1)]);
                
                f1p1_B = (TAMask_B[(g1+1)*W1+g2]) ? F_B[(g1+1)*W1+g2] : F_B[g1*W1+g2];
                f1m1_B = (TAMask_B[(g1-1)*W1+g2]) ? F_B[(g1-1)*W1+g2] : F_B[g1*W1+g2];
                f2p1_B = (TAMask_B[g1*W1+(g2+1)]) ? F_B[g1*W1+(g2+1)] : F_B[g1*W1+g2];
                f2m1_B = (TAMask_B[g1*W1+(g2-1)]) ? F_B[g1*W1+(g2-1)] : F_B[g1*W1+g2];

                b1 = PF_B[g1*W1+g2] >= TolL_B;
                b2 = ((nx1 == 0) ? 0.0 : pow(std::abs(f1p1_B - f1m1_B)/(nx1*H[0]),2)) + \
                     ((nx2 == 0) ? 0.0 : pow(std::abs(f2p1_B - f2m1_B)/(nx2*H[1]),2)) >= TolLd_B_sq;
                b3 = g1 > EDGE && g2 > EDGE;
                b4 = (g1 < BoxShape[0]-EDGE-1) && (g2 < BoxShape[1]-EDGE-1);
        ...
        while ( !isFullGrid && (TBL_A.size() != 0 || TBL_B.size() != 0) && (Excount_A < ExLimit && Excount_B < ExLimit) )
        {
                    ...
                    for (int i1 = x1_min_C; i1 <= x1_max_C; i1 ++)  {
                        for (int i2 = x2_min_C; i2 <= x2_max_C; i2 ++)  {
                            if (TAMask_A[i1*W1+i2] || TAMask_B[i1*W1+i2])  {
                                ...
                                kk1p1_A = KK1_A[(i1+1)*W1+i2];
                                kk1m1_A = KK1_A[(i1-1)*W1+i2];
                                ...
                                kk1p1_B = KK1_B[(i1+1)*W1+i2];
                                kk1m1_B = KK1_B[(i1-1)*W1+i2];
                    ...
                    for (int i1 = x1_min_C; i1 <= x1_max_C; i1 ++)  {
                        for (int i2 = x2_min_C; i2 <= x2_max_C; i2 ++)  {
                            if (TAMask_A[i1*W1+i2] || TAMask_B[i1*W1+i2])  {
                                ...
                                kk1p1_A = KK2_A[(i1+1)*W1+i2];
                                kk1m1_A = KK2_A[(i1-1)*W1+i2];
                                ...
                                kk1p1_B = KK2_B[(i1+1)*W1+i2];
                                kk1m1_B = KK2_B[(i1-1)*W1+i2];
                    ...
                    for (int i1 = x1_min_C; i1 <= x1_max_C; i1 ++)  {
                        for (int i2 = x2_min_C; i2 <= x2_max_C; i2 ++)  {
                            if (TAMask_A[i1*W1+i2] || TAMask_B[i1*W1+i2])  {
                                ...
                                kk1p1_A = KK3_A[(i1+1)*W1+i2];
                                kk1m1_A = KK3_A[(i1-1)*W1+i2];
                                ...
                                kk1p1_B = KK3_B[(i1+1)*W1+i2];
                                kk1m1_B = KK3_B[(i1-1)*W1+i2];
            ...
            else
            {
                ...
                if ( ExFF_A.size() > 0 )  {
                    ...
                    #pragma omp parallel for reduction(merge: ExBD_A) private(g1,g2,n1,n2) schedule(runtime)
                    for (int i = 0; i < ExFF_A.size(); i++)
                    {
                        if (Check_A[i])  
                        {
                ...
                if ( ExFF_B.size() > 0 )  {
                    ...
                    #pragma omp parallel for reduction(merge: ExBD_B) private(g1,g2,n1,n2) schedule(runtime)
                    for (int i = 0; i < ExFF_B.size(); i++)  {
                        if (Check_B[i])  
                        {
                            g1 = (int)(ExFF_B[i] / M1);
                            ...
                            for (int j = 0; j < nneigh; j ++)  {
                                ...
                                if (TAMask_B[(g1+n1)*W1+(g2+n2)])
                                    ExBD_B.push_back(GridToIdx(g1+n1,g2+n2));
                            }
                        }
                    }
                ...
                if ( ExFF_A.size() > 0 || ExFF_B.size() > 0 )  {
                    ...
                    for (int i = 0; i < ExBD.size(); i++)  {
                        ...
                        kk1p1_A = KK1_A[(g1+1)*W1+g2];
                        kk1m1_A = KK1_A[(g1-1)*W1+g2];
                        ...
                        kk1p1_B = KK1_B[(g1+1)*W1+g2];
                        kk1m1_B = KK1_B[(g1-1)*W1+g2];
                    ...
                    for (int i = 0; i < ExBD.size(); i++)  {
                        ...
                        kk1p1_A = KK2_A[(g1+1)*W1+g2];
                        kk1m1_A = KK2_A[(g1-1)*W1+g2];
                        ...
                        kk1p1_B = KK2_B[(g1+1)*W1+g2];
                        kk1m1_B = KK2_B[(g1-1)*W1+g2];
                    ...
                    for (int i = 0; i < ExBD.size(); i++)  {
                        ...
                        kk1p1_A = KK3_A[(g1+1)*W1+g2];
                        kk1m1_A = KK3_A[(g1-1)*W1+g2];
                        ...
                        kk1p1_B = KK3_B[(g1+1)*W1+g2];
                        kk1m1_B = KK3_B[(g1-1)*W1+g2];
            ...
            if ( ExFF_A.size() > 0 )  {
                ...
                #pragma omp parallel for reduction(merge: tmpVec) private(g1,g2,b1,b2,b3,b4,nx1,nx2,\
                                                                          f0_A,f1p1_A,f1m1_A,f2p1_A,f2m1_A) schedule(runtime)
                for (int i = 0; i < ExFF_A.size(); i++)
                {
                    if (Check_A[i])  {
                        ...
                        f1p1_A = (TAMask_A[(g1+1)*W1+g2]) ? FF_A[(g1+1)*W1+g2] : FF_A[g1*W1+g2];
                        f1m1_A = (TAMask_A[(g1-1)*W1+g2]) ? FF_A[(g1-1)*W1+g2] : FF_A[g1*W1+g2];
                        f2p1_A = (TAMask_A[g1*W1+(g2+1)]) ? FF_A[g1*W1+(g2+1)] : FF_A[g1*W1+g2];
                        f2m1_A = (TAMask_A[g1*W1+(g2-1)]) ? FF_A[g1*W1+(g2-1)] : FF_A[g1*W1+g2];
            ...
            if ( ExFF_B.size() > 0 )  {
                ...
                #pragma omp parallel for reduction(merge: tmpVec) private(g1,g2,b1,b2,b3,b4,nx1,nx2,\
                                                                        f0_B,f1p1_B,f1m1_B,f2p1_B,f2m1_B) schedule(runtime)
                for (int i = 0; i < ExFF_B.size(); i++)  
                {
                    if (Check_B[i])  {
                        ...
                        f1p1_B = (TAMask_B[(g1+1)*W1+g2]) ? FF_B[(g1+1)*W1+g2] : FF_B[g1*W1+g2];
                        f1m1_B = (TAMask_B[(g1-1)*W1+g2]) ? FF_B[(g1-1)*W1+g2] : FF_B[g1*W1+g2];
                        f2p1_B = (TAMask_B[g1*W1+(g2+1)]) ? FF_B[g1*W1+(g2+1)] : FF_B[g1*W1+g2];
                        f2m1_B = (TAMask_B[g1*W1+(g2-1)]) ? FF_B[g1*W1+(g2-1)] : FF_B[g1*W1+g2];
        ...
        if ( (tt + 1) % PERIOD == 0 )
        {
            ...
            if (isTrans)  {
                ...
                pftrans *= H[0] * H[1];
                log->log("[Nonadiabatic2d] Time %lf, Trans_A = %.16e\n", ( tt + 1 ) * kk, pftrans);
                ...
                pftrans *= H[0] * H[1];
                log->log("[Nonadiabatic2d] Time %lf, Trans_B = %.16e\n", ( tt + 1 ) * kk, pftrans);
            ...
            if (isPrintAdiaDensity)  {
                ...
                t_full += t_1_elapsed;
                t_truncate += t_1_elapsed;
                if (!QUIET && TIMING) log->log("Elapsed time (omp-x-2 adiabatic) = %.4e sec\n", t_1_elapsed); 
            }
            // ----------------------------------------------------------------------------
        }
        ...
        if ( !isFullGrid )
        {
                ...
                for (int i2 = x2_min_C; i2 <= x2_max_C; i2 ++)  {
                    if (TAMask_A[i1*W1+i2])  {
                        if (PF_A[i1*W1+i2] < TolH_A)  {
                            ...
                            f1p1_A = (TAMask_A[(i1+1)*W1+i2]) ? FF_A[(i1+1)*W1+i2] : FF_A[i1*W1+i2];
                            f1m1_A = (TAMask_A[(i1-1)*W1+i2]) ? FF_A[(i1-1)*W1+i2] : FF_A[i1*W1+i2];
                            f2p1_A = (TAMask_A[i1*W1+(i2+1)]) ? FF_A[i1*W1+(i2+1)] : FF_A[i1*W1+i2];
                            f2m1_A = (TAMask_A[i1*W1+(i2-1)]) ? FF_A[i1*W1+(i2-1)] : FF_A[i1*W1+i2];
                ...
                for (int i2 = x2_min_C; i2 <= x2_max_C; i2 ++)  {
                    if (TAMask_B[i1*W1+i2])  {
                       if (PF_B[i1*W1+i2] < TolH_B)  {
                            ...
                            f1p1_B = (TAMask_B[(i1+1)*W1+i2]) ? FF_B[(i1+1)*W1+i2] : FF_B[i1*W1+i2];
                            f1m1_B = (TAMask_B[(i1-1)*W1+i2]) ? FF_B[(i1-1)*W1+i2] : FF_B[i1*W1+i2];
                            f2p1_B = (TAMask_B[i1*W1+(i2+1)]) ? FF_B[i1*W1+(i2+1)] : FF_B[i1*W1+i2];
                            f2m1_B = (TAMask_B[i1*W1+(i2-1)]) ? FF_B[i1*W1+(i2-1)] : FF_B[i1*W1+i2];
            ...
            if (ta_size_A == 0)
                ...
                for (int i1 = x1_min_C; i1 <= x1_max_C; i1 ++)  {
                    for (int i2 = x2_min_C; i2 <= x2_max_C; i2 ++)  {
                        if (TAMask_A[i1*W1+i2])  {
                            if (!TAMask_A[(i1-1)*W1+i2] || !TAMask_A[(i1+1)*W1+i2] || \
                                !TAMask_A[i1*W1+(i2-1)] || !TAMask_A[i1*W1+(i2+1)])
                                tmpVec.push_back(i1*W1+i2);
                        }
                    }
            ...
            if (ta_size_B == 0)
                ...
                for (int i1 = x1_min_C; i1 <= x1_max_C; i1 ++)  {
                    for (int i2 = x2_min_C; i2 <= x2_max_C; i2 ++)  {
                        if (TAMask_B[i1*W1+i2])  {
                            if (!TAMask_B[(i1-1)*W1+i2] || !TAMask_B[(i1+1)*W1+i2] || \
                                !TAMask_B[i1*W1+(i2-1)] || !TAMask_B[i1*W1+(i2+1)])
                                tmpVec.push_back(i1*W1+i2);
                        }
                    }
