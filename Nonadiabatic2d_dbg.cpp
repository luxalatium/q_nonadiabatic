...
void Nonadiabatic2d::Evolve()
{
        ...
        for (int i1 = 0; i1 < BoxShape[0]; i1 ++)  {
            for (int i2 = EDGE; i2 < BoxShape[1]-EDGE; i2 ++)  {
                ...
                if (b1 && b2)  {
                    ...
                    if (b1 && !b2)  {
                        log->log("[TEST-TG1-A-%d] %.12e %.12e\n",0,Box[0]+i1*H[0],Box[2]+i2*H[1]);
                    }
                    if (!b1)  {
                        log->log("[TEST-TG2-A-%d] %.12e %.12e\n",0,Box[0]+i1*H[0],Box[2]+i2*H[1]);
                    }
                }
            }
        }
        ...
        for (int i1 = 0; i1 < BoxShape[0]; i1 ++)  {
            for (int i2 = EDGE; i2 < BoxShape[1]-EDGE; i2 ++)  {
                ...
                if (b1 && b2)  {
                    ...
                    if (b1 && !b2)  {
                        log->log("[TEST-TG1-B-%d] %.12e %.12e\n",0,Box[0]+i1*H[0],Box[2]+i2*H[1]);
                    }
                    if (!b1)  {
                        log->log("[TEST-TG2-B-%d] %.12e %.12e\n",0,Box[0]+i1*H[0],Box[2]+i2*H[1]);
                    }
                }
            }
        }
    ...
    for (int tt = 0; tt < (int)(TIME / kk); tt ++)
    {
		...
		//if (TIMING)  {
                	//log->log("[Nonadiabatic2d] TEST 1 TA Range (A) [%d, %d][%d, %d]\n", x1_min_A, x1_max_A, x2_min_A, x2_max_A);
                	//log->log("[Nonadiabatic2d] TEST 1 TA Range (B) [%d, %d][%d, %d]\n", x1_min_B, x1_max_B, x2_min_B, x2_max_B);
		//}
		    ...
		    //if (TIMING)  {
                    	//log->log("[Nonadiabatic2d] TEST 2 TA Range (A) [%d, %d][%d, %d]\n", x1_min_A, x1_max_A, x2_min_A, x2_max_A);
                    	//log->log("[Nonadiabatic2d] TEST 2 TA Range (B) [%d, %d][%d, %d]\n", x1_min_B, x1_max_B, x2_min_B, x2_max_B);
		    //}
	...
	//if (TIMING)  {
            //log->log("[Nonadiabatic2d] TEST 3 TA Range (A) [%d, %d][%d, %d]\n", x1_min_A, x1_max_A, x2_min_A, x2_max_A);
            //log->log("[Nonadiabatic2d] TEST 3 TA Range (B) [%d, %d][%d, %d]\n", x1_min_B, x1_max_B, x2_min_B, x2_max_B);

            //for (int i1 = x1_min_C; i1 <= x1_max_C; i1 ++)  {
                //for (int i2 = x2_min_C; i2 <= x2_max_C; i2 ++)  {
                    //if (TAMask_B[i1*W1+i2])  {
                        //log->log("[MASK-1-%d] %d %d\n", tt, i1, i2);
                    //}
                //}
            //}
	//}
        ...
        if ( !isFullGrid )
        {
            ...
            #pragma omp parallel for reduction(min: x1_min_T,x2_min_T) reduction(max: x1_max_T,x2_max_T) reduction(+: ta_size_A,ta_size_B) schedule(runtime)
            for (int i1 = x1_min_C; i1 <= x1_max_C; i1 ++)  {
                for (int i2 = x2_min_C; i2 <= x2_max_C; i2 ++)  {
                    if (TAMask_A[i1*W1+i2] || TAMask_B[i1*W1+i2])  {
                        ...
                        if (TAMask_A[i1*W1+i2]) ta_size_A += 1;
                        if (TAMask_B[i1*W1+i2]) ta_size_B += 1;
                    }
                }
            }
	    ...
	    //if (TIMING)  {
                //log->log("[Nonadiabatic2d] TEST 4 TA Range (A) [%d, %d][%d, %d]\n", x1_min_A, x1_max_A, x2_min_A, x2_max_A);
                //log->log("[Nonadiabatic2d] TEST 4 TA Range (B) [%d, %d][%d, %d]\n", x1_min_B, x1_max_B, x2_min_B, x2_max_B);
	    //}
            t_1_end = omp_get_wtime();
            t_1_elapsed = t_1_end - t_1_begin;
            t_overhead += t_1_elapsed;
